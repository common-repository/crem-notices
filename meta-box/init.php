...
spl_autoload_register('crno_cmb_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'crno_cmb_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new crno_cmb_Meta_Box( $meta_box );
}

define( 'CMB_META_BOX_URL', crno_cmb_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class crno_cmb_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'crno_cmb_styles' => true, // Include cmb bundled stylesheet
		'fields'     => array(),
	);
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'crno_cmb_allow_frontend', true, $meta_box );
		...
		$show_filters = 'crno_cmb_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'crno_cmb_show_on', array( $show_filters, $filter ), 10, 2 );
		}
	...
	public function register_scripts() {
		...
		wp_localize_script( 'cmb-scripts', 'crno_cmb_l10', apply_filters( 'crno_cmb_localized_data', array(
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( $this->_meta_box['crno_cmb_styles'] )
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'crno_cmb_show_on', true, $this->_meta_box ) )
	...
	public function user_metabox() {
		...
		if ( $this->_meta_box['crno_cmb_styles'] != false )
			wp_enqueue_style( 'cmb-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		echo "\n<!-- Begin CMB Fields -->\n";
		do_action( 'crno_cmb_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table crno_cmb_metabox">';
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new crno_cmb_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
		do_action( 'crno_cmb_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CMB Fields -->\n";
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new crno_cmb_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				...
				if ( $desc )
					echo '<p class="crno_cmb_metabox_description">'. $desc .'</p>';
			echo '</th></tr>';
		}
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new crno_cmb_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'crno_cmb_show_on', true, $meta_box ) )
			return;
		...
		do_action( "crno_cmb_save_{$object_type}_fields", $object_id, $meta_box['id'], self::$updated, $meta_box );
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$crno_cmb_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
		  $crno_cmb_url = str_replace(
		...
		return trailingslashit( apply_filters('crno_cmb_meta_box_url', $crno_cmb_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'crno_cmb_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "crno_cmb_override_option_get_$option_key", 'crno_cmb_no_override_option_get', $default );

		if ( $test_get !== 'crno_cmb_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'crno_cmb_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "crno_cmb_override_option_save_$option_key", 'crno_cmb_no_override_option_save', $to_save );

		if ( $test_save !== 'crno_cmb_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_crno_cmb_oembed_handler', array( 'crno_cmb_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_crno_cmb_oembed_handler', array( 'crno_cmb_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function crno_cmb_get_option( $option_key, $field_id = '' ) {
	return crno_cmb_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              crno_cmb_Meta_Box_field object
 */
function crno_cmb_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	crno_cmb_Meta_Box::set_object_id( $object_id );
	crno_cmb_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new crno_cmb_Meta_Box_field( $field_args );
}
 ...
 */
function crno_cmb_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = crno_cmb_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function crno_cmb_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		crno_cmb_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function crno_cmb_print_metabox( $meta_box, $object_id ) {
	$cmb = new crno_cmb_Meta_Box( $meta_box );
	if ( $cmb ) {
		...
		if ( $meta_box['crno_cmb_styles'] != false )
			wp_enqueue_style( 'cmb-styles' );

		crno_cmb_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function crno_cmb_save_metabox_fields( $meta_box, $object_id ) {
	crno_cmb_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function crno_cmb_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = crno_cmb_Meta_Box::set_mb_defaults( $meta_box );
	...
	crno_cmb_Meta_Box::set_object_type( crno_cmb_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], crno_cmb_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		crno_cmb_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'crno_cmb_frontend_form_format', '<form class="cmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cmb" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );
